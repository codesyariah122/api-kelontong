...
class TokoKelontongController extends Controller
{
    ...
    public function detect_ip(Request $request)
    {
        try{
            $ip = $this->getIp();
            dd($ip);
    ...
    public function location(Request $request)
    {
        try{
            ...
            $location = Http::get("https://api.ipfind.com?ip={$ip}&auth={$ip_find_key}");
    ...
    {
        try{
            $list_visitor = Visitor::all();
            return response()->json([
                'message' => 'Fetch visitor data',
                'data' => $list_visitor
            ], 401);
        }catch(Exception $e){
            return response()->json([
